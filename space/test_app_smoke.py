...
def test_space_app_source_records_startup_diagnostics():
    source = Path(__file__).with_name("app.py").read_text(encoding="utf-8")

    assert "ARL_SPACE_STARTUP_LOG" in source
    assert "module_import_started" in source
    assert "mcp_store_helpers_decoupled" in source
