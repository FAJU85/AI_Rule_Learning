...
_log = logging.getLogger("arl.space")
_STARTUP_LOG_PATH = Path(os.environ.get("ARL_SPACE_STARTUP_LOG", "/tmp/ai-rule-learning-space-startup.jsonl"))


def _record_startup_event(event: str, **fields: Any) -> None:
    payload = {
        "event": event,
    ...
    try:
        ...
        _log.warning("space startup log write failed: %s", exc)


_record_startup_event("module_import_started")
...
_record_startup_event(
    "mcp_store_helpers_decoupled",
    find_rule_health_candidates="local_fallback",
    suggest_duplicate_rules_from_records="local_fallback",
)
