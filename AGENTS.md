AGENTS.md — Universal AI Engineering Operating Standard
...
2. Order of precedence when instructions conflict:
   User instruction (current turn) > AGENTS.md > Agent's training defaults
3. If any rule is unclear or conflicts with a user request, do not silently reinterpret. Surface the
   conflict and defer (see Handling Ambiguity, Failure & Non-Recoverable State).
4. This standard is framework-agnostic, profile-driven, and tool-aware. All tool choices are derived
   from project profile and language ecosystem.
5. Reference data (capability definitions, tool matrices, validation criteria) are maintained in
   separate files under .agents/ (see Reference Files).
...
Project Name ai-rule-learning Repository name
Primary Profile Hugging Face Space From Profile Matrix
Language(s) Python Primary language(s)
Runtime(s) Python 3.11 Runtime environment(s)
Package Manager pip Dependency manager
Default Branch main Main development branch
Deployment Target Hugging Face Spaces Production environment
Coverage Threshold 80 Minimum test coverage %
Branch Prefixes feat, fix, chore, docs, style, refactor, perf, test, build, ci, revert Allowed prefixes
...
· Modify AGENTS.md without explicit instruction.
...
```bash
ruff format --check .       # format:check (Python)
npm run format:check        # format:check (Markdown / JSON / YAML)
ruff check .                # lint
mypy src                    # typecheck
pytest                      # test
pytest tests/e2e            # test:e2e — only if changes affect critical flows
```

All must pass. Coverage must stay ≥ 80% (see Validation Matrix).
...
Defines engineering capabilities independent of tools. Tools are assigned in the Tool Matrix based on
language and profile. See .agents/capabilities.md for complete definitions.
...
Maps project types to required capabilities using explicit notation:
Required/X where X = total capabilities in that group.

Legend: R/X = Required (X of X capabilities), O = Optional, N/A = Not Applicable

Profile Code Quality Testing Security Reliability Observability Performance Accessibility Docs AI/LLM
...
To keep AGENTS.md focused on policy and workflow, reference data is maintained in separate files:
...
pytest tests/e2e          # CI-compatible

# Build & Deploy (if automated)
# (deployment is handled by Hugging Face Spaces on push)

# Security
pip-audit
gitleaks detect --source .   # if configured
```

Human-Only (Requires GUI/Manual)

```bash
npm run release:*            # Version bumps (operator only)
```
...
The agent must never invoke release:\* commands directly. If a release is required, the agent opens a
draft PR with title `chore(release): proposed <version>` containing the changelog diff and tags
@maintainer for human approval. The agent then proceeds with other work; release execution is the
...
· Tests live next to source (foo.py → foo_test.py) or in mirrored tests/ directory. Pick one and be consistent.
...
· Use feat, fix, chore, docs, style, refactor, perf, test, build, ci, revert/<short-slug> branches (e.g., feat/user-onboarding)
...
Unrecoverable component Quarantine: isolate, mark `// AGENT-QUARANTINE: <reason> <date>`,
ensure rest passes, defer in report
Conflict user instruction vs AGENTS.md Defer; surface conflict; let user resolve
Scope creep Complete original task; list additional work in report; don't expand silently

Do not halt for small ambiguities. Halting reserved for: missing credentials, destructive
operations, off-limits file edits, explicit conflicts.
...
npm install                     # for markdown/json/yml tooling
...
If the agent encounters recurring ambiguity not covered here, propose an amendment in the
End-of-Session Report — phrased as a concrete clause with section. User accepts or rejects;
agent does not self-edit.
...
· Branch prefixes placeholder (Task 2.3)
