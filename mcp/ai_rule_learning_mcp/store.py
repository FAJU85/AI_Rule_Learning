...
def offline_enabled() -> bool:
    """Return True when outbound network features must be disabled."""
...
def _hf_api():
    if offline_enabled():
        raise RuntimeError("ARL_OFFLINE=true disables Hugging Face network access")
...
def hf_enabled() -> bool:
    ...
    return bool(HF_DATASET and HF_TOKEN and not offline_enabled())
...
def _upload(filename: str, records: list[dict]) -> None:
    ...
    if offline_enabled() or not HF_DATASET or not HF_TOKEN:
        return
    content = "\n".join(json.dumps(r, ensure_ascii=False) for r in records) + "\n"
    try:
        _hf_api().upload_file(
...
def _rule_comparison_text(rule: dict) -> str:
    ...
    return " ".join(
        ...
        for part in (
            ...
            rule.get("instruction", ""),
            action_instruction,
            " ".join(rule.get("triggers", []))
            if isinstance(rule.get("triggers"), list)
            else str(rule.get("triggers", "")),
        )
        if part
    ).lower()
...
def _rule_tokens(rule: dict) -> set[str]:
    return {
        token
        for token in re.findall(r"[a-z0-9_]{3,}", _rule_comparison_text(rule))
...
def suggest_duplicate_rules_from_records(
    ...
    tokenized = [(rule, _rule_tokens(rule)) for rule in candidates]
